...
class _RiskScreenState extends State<RiskScreen> {
  ...
  // ── Score Card — TTS button on the header row ─────────────────────────────────
  Widget _buildScoreCard(UserProfile profile) {
    final score = profile.strokeRiskScore;
    final riskLevel = profile.strokeRiskLevel;
    final color = AppTheme.riskColor(riskLevel);
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      // ← ADDED: TTS button reads out the full risk score summary
                      TtsButton(
        ...
        children: [
          ...
          Text(
            ...
            style: GoogleFonts.inter(
              ...
              fontWeight: FontWeight.w700,
              color:
                  item.active ? AppTheme.primary : AppTheme.textSecondary,
            ),
          ),
        ],
      ),
  ...
  // ── Recommendations — TTS button on header + per item ────────────────────────
  Widget _buildRecommendations(UserProfile profile) {
    final recommendations =
        RiskService.getRecommendations(profile, _measurements);
        ...
        children: [
          Row(
            ...
            children: [
              ...
              // ← ADDED: "Read all" button reads every recommendation at once
              TtsButton(
                onSpeak: () =>
  ...
  // ← ADDED: TtsButton per recommendation item
  Widget _buildRecommendationItem(int number, String text) {
    return Padding(
      padding: const EdgeInsets.only(bottom: 16),
      child: Row(
        ...
        children: [
          ...
          TtsButton(
            size: 18,
            onSpeak: () =>
                _ttsService.speakSingleRecommendation(number, text),
          ),
        ],
      ),
    );
