...
//     }
//     await PinService.saveDob(selectedDob); // save DOB for future PIN resets
//   }
...
import '../models/user_profile.dart';
import '../services/hive_service.dart';
import '../services/pin_service.dart';   // ← ADD THIS
import '../theme/app_theme.dart';
import '../utils/validators.dart';
...
class _OnboardingScreenState extends State<OnboardingScreen> {
  ...
  Future<void> _selectDateOfBirth() async {
    ...
    if (picked != null) {
      setState(() => _dateOfBirth = picked);
      await PinService.saveDob(picked); // ← FIXED: use 'picked', not 'selectedDob'
    }
  }
  ...
  Future<void> _saveProfile() async {
    ...
    await _hiveService.saveUserProfile(profile);
    if (!mounted) return;
    // ← FIXED: go to pin-setup, not /home — user must create PIN before entering app
