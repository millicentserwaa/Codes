...
float readBatteryPercent() {
  // ESP32 ADC requires explicit attenuation for accurate readings above 1V
  // ADC_11db allows 0–3.9V input range on GPIO35
  analogSetPinAttenuation(BATT_ADC_PIN, ADC_11db);

  // Average 20 readings to reduce ESP32 ADC noise
  long sum = 0;
  for (int i = 0; i < 20; i++) {
  ...
  // ESP32 ADC with 11dB attenuation: full scale ~3.9V at 4095
  // x2 because equal resistor divider halves the battery voltage
  float voltage = (raw / 4095.0f) * 3.9f * 2.0f;

  // Map 3.2V (0%) to 4.2V (100%)
  float percent = ((voltage - 3.2f) / (4.2f - 3.2f)) * 100.0f;
  percent = constrain(percent, 0.0f, 100.0f);

  Serial.print("Battery ADC raw: "); Serial.print((int)raw);
