        ...
        namespace Port {
            class RandomForest {
                public:
                    ...
                    /**
                     * Predict and expose raw vote counts so the caller
                     * can compute real confidence from the forest.
                     *
                     * Usage:
                     *   uint8_t votes[2] = {0};
                     *   int prediction = clf.predictWithVotes(features, votes);
                     *   int confidence = (int)((float)votes[prediction] / 7.0f * 100.0f);
                     *
                     * votes[0] = number of trees that voted NORMAL  (out of 7)
                     * votes[1] = number of trees that voted AF       (out of 7)
                     *
                     * Examples:
                     *   7/7 AF votes  → confidence 100%  (unanimous)
                     *   5/7 AF votes  → confidence  71%  (strong)
                     *   4/7 AF votes  → confidence  57%  (marginal — borderline case)
                     */
                    int predictWithVotes(float *x, uint8_t *votesOut) {
                        uint8_t votes[2] = { 0 };
                        _runTrees(x, votes);

                        // Expose votes to caller
                        votesOut[0] = votes[0];
                        votesOut[1] = votes[1];
                ...
                protected:
                    ...
                    /**
                     * Internal: run all 7 trees and fill the votes array.
                     * Both predict() and predictWithVotes() call this so
                     * the tree logic lives in exactly one place.
                     */
